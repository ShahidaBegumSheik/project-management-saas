...
def test_register_creates_user_subscription_and_token(client, db_session):
    resp = client.post(
    ...
    audit_rows = db_session.query(AuditLog).filter(AuditLog.actor_user_id == user.id).all()
...
def test_login_success_returns_token(client):
    ...
    resp = client.post("/api/v1/auth/login-json", json={"email": "bob@example.com", "password": "password123"})
...
def test_login_invalid_credentials_fails(client):
    ...
    resp = client.post("/api/v1/auth/login-json", json={"email": "eve@example.com", "password": "wrongpass"})
...
def test_login_success_returns_token_using_oauth_form(client):
    client.post("/api/v1/auth/register", json={"email": "form@example.com", "password": "password123"})
    resp = client.post("/api/v1/auth/login", data={"username": "form@example.com", "password": "password123"})
    assert resp.status_code == 200, resp.text
    assert "access_token" in resp.json()